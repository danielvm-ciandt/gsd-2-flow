...
function getGeminiCliClientId(): string {
	const id = process.env.GOOGLE_GEMINI_CLI_OAUTH_CLIENT_ID;
	if (!id) {
		throw new Error(
			"Missing GOOGLE_GEMINI_CLI_OAUTH_CLIENT_ID environment variable (required for Gemini CLI OAuth).",
		);
	}
	return id;
}
...
function getGeminiCliClientSecret(): string {
	const secret = process.env.GOOGLE_GEMINI_CLI_OAUTH_CLIENT_SECRET;
	if (!secret) {
		throw new Error(
			"Missing GOOGLE_GEMINI_CLI_OAUTH_CLIENT_SECRET environment variable (required for Gemini CLI OAuth).",
		);
	}
	return secret;
}
...
export async function refreshGoogleCloudToken(refreshToken: string, projectId: string): Promise<OAuthCredentials> {
	return refreshGoogleOAuthToken(refreshToken, getGeminiCliClientId(), getGeminiCliClientSecret(), "Google Cloud", { projectId });
}
...
export async function loginGeminiCli(
	...
	try {
		...
		const authParams = new URLSearchParams({
			client_id: getGeminiCliClientId(),
			response_type: "code",
		...
		const tokenResponse = await fetch(TOKEN_URL, {
			...
			body: new URLSearchParams({
				client_id: getGeminiCliClientId(),
				client_secret: getGeminiCliClientSecret(),
				code,
