...
function getAntigravityClientId(): string {
	const id = process.env.GOOGLE_ANTIGRAVITY_OAUTH_CLIENT_ID;
	if (!id) {
		throw new Error(
			"Missing GOOGLE_ANTIGRAVITY_OAUTH_CLIENT_ID environment variable (required for Antigravity OAuth).",
		);
	}
	return id;
}
...
function getAntigravityClientSecret(): string {
	const secret = process.env.GOOGLE_ANTIGRAVITY_OAUTH_CLIENT_SECRET;
	if (!secret) {
		throw new Error(
			"Missing GOOGLE_ANTIGRAVITY_OAUTH_CLIENT_SECRET environment variable (required for Antigravity OAuth).",
		);
	}
	return secret;
}
...
export async function refreshAntigravityToken(refreshToken: string, projectId: string): Promise<OAuthCredentials> {
	return refreshGoogleOAuthToken(refreshToken, getAntigravityClientId(), getAntigravityClientSecret(), "Antigravity", { projectId });
}
