...
describe("Antigravity OAuth — credential regression", () => {
	...
	test("credentials are loaded from environment variables", () => {
		...
		assert.ok(content.includes("process.env.GOOGLE_ANTIGRAVITY_OAUTH_CLIENT_ID"));
		assert.ok(content.includes("process.env.GOOGLE_ANTIGRAVITY_OAUTH_CLIENT_SECRET"));
