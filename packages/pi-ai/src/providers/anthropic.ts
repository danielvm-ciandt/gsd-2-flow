...
/**
 * True when the active credential came from ANTHROPIC_AUTH_TOKEN
 * (e.g. a CI&T Flow proxy JWT), which requires Bearer auth.
 */
export function isFlowAuthToken(): boolean {
...
async function createClient(
	...
	// Also skip when CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1 (e.g. Flow/Bedrock proxy).
	...
	const usesBearerAuth = usesAnthropicBearerAuth(model.provider) || hasBearerAuthorizationHeader(model) || isFlowAuthToken();
	const client = new AnthropicClass({
