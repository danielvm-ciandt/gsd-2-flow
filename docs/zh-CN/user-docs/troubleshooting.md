...
**症状：** `npm install -g github:danielvm-ciandt/gsd-2-flow` 成功，但系统找不到 `gsd`。
...
**临时方案：** 直接执行 `npm exec -g -- gsd`，或使用 `$(npm prefix -g)/bin/gsd`。
...
### `npm install -g github:danielvm-ciandt/gsd-2-flow` 失败
